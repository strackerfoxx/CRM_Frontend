...
export default function FieldChoiceCard({services, servicesSelected, setServicesSelected}) {
  ...
  return (
    <div className="w-full">
      <FieldGroup>
        <FieldSet className="max-h-[15rem] border rounded-lg bg-card overflow-y-auto gap-3 p-4">
            {services && services.map((service) => (
                <FieldLabel htmlFor={service.id} key={service.id} className="cursor-pointer hover:bg-accent hover:text-accent-foreground rounded-md">
