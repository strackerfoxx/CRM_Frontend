...
export default function BlockedTimesSection() {
    ...
    const fetchBlockedTimes = async () => {
        setIsLoading(true)
        try {
            const { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/blocked-time/get-blocked-times`, {
                headers: {
                    Authorization: token,
                },
            })
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEditing) {
                payload.id = editId
                await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/blocked-time/update`, payload, {
                    headers: { Authorization: token },
                })
                toast.success("Tiempo bloqueado actualizado")
            } else {
                await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/blocked-time/create`, payload, {
                    headers: { Authorization: token },
                })
